package me.jtx.robinia.payment.unionpay.service.impl;
...
import me.jtx.robinia.payment.unionpay.util.UnionConfig;
import me.jtx.robinia.payment.util.CommonUtil;
...
public class UnionPayServiceImpl implements IUnionPayService {
    ...
    /* (non-Javadoc)
     * @see me.jtx.robinia.payment.unionpay.service.IUnionPayService#pay(me.jtx.robinia.payment.model.Goods)
     */
    @Override
    public String pay(Goods goods) {
        ...
        if (goods.getPayWay() == PayWay.MOBILE.getCode()) {// 手机支付
        ...
        requestData.put("orderId", goods.getOutTradeNo()); // 商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
        requestData.put("txnTime", UnionConfig.getCurrentTime()); // 订单发送时间，取系统时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
        requestData.put("currencyCode", "156"); // 交易币种（境内商户一般是156 人民币）
        requestData.put("txnAmt", CommonUtil.subZeroAndDot(goods.getTotalFee())); // 交易金额，单位分，不要带小数点
    ...
    /* (non-Javadoc)
     * @see me.jtx.robinia.payment.unionpay.service.IUnionPayService#handNotify(java.util.Map, java.lang.String)
     */
    @Override
    public String handNotify(Map<String, String> valideData, String encoding) {
    ...
    /* (non-Javadoc)
     * @see me.jtx.robinia.payment.unionpay.service.IUnionPayService#fileTransfer()
     */
    @Override
